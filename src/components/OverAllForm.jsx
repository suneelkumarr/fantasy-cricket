...
function OverAllForm({ data, matchInSights, playerInfo }) {
  ...
  const getBarWidth = (points) => {
    ...
    const maxPoints = Math.max(
      ...data.stats_data.form.recent_stats.map((item) =>
  ...
  const overAllForm = data?.stats_data?.form?.recent_stats;

  return (
    <div>
      <h2 className="text-xl font-semibold text-center text-gray-700">
        Last {data.stats_data.graph.recent_stats.dream_team?.total_matches}{" "}
      ...
      <div className="max-w-3xl mx-auto p-4 font-sans">
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            <div className="relative h-6 bg-gray-200 rounded-full overflow-hidden">
              <div
                className={`absolute top-0 left-0 h-full rounded-l-full ${(() => {
                  const value =
                    data.stats_data.graph.recent_stats.dream_team.value;
                  const maxMatches =
                    data.stats_data.graph.recent_stats.dream_team.total_matches;
                ...
                style={{
                  width: `${
                    (data.stats_data.graph.recent_stats.dream_team.value /
                      data.stats_data.graph.recent_stats.dream_team
                        .total_matches) *
                    100
                  }%`,
                }}
            ...
            <div className="text-center mt-1 text-sm font-semibold">
              {data.stats_data.graph.recent_stats.dream_team?.value} Times
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative h-6 bg-gray-200 rounded-full overflow-hidden">
              <div
                className={`absolute top-0 left-0 h-full rounded-l-full ${(() => {
                  const value =
                    data.stats_data.graph.recent_stats.underperformed.value;
                  const maxMatches =
                    data.stats_data.graph.recent_stats.underperformed
                      .total_matches;
            ...
            <div className="text-center mt-1 text-sm font-semibold">
              {data.stats_data.graph.recent_stats.underperformed?.value} Times
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            <div className="text-center shadow-md h-20">
              <div className="text-4xl font-bold text-gray-800 pt-2">
                {data.stats_data.graph.recent_stats.bat_first_fpts?.value}
              </div>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            <div className="text-center shadow-md h-20">
              <div className="text-4xl font-bold text-gray-800 pt-2">
                {data.stats_data.graph.recent_stats.bowl_first_fpts?.value}
              </div>
              <div className="text-xs text-gray-500">Avg FPts</div>
            </div>
          </div>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            <div className="text-center shadow-md h-20">
              <div className="text-4xl font-bold text-gray-800 pt-2">
                {data.stats_data.graph.recent_stats.avg_position_rank?.value}
              </div>
              <div className="text-xs text-gray-500">
                /{" "}
                {
                  data.stats_data.graph.recent_stats.avg_position_rank
                    ?.graph_total
                }{" "}
